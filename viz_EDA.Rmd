...
weather_df =
  ...
  mutate(
    ...
    tmin = tmin / 10,
    tmax = tmax / 10,
    ## take the date and round it
    month = lubridate::floor_date(date, unit = "month")) %>%
  select(name, id, date, month, everything())
```

## `group_by` and and counting
## code chunk specific
```{r}
## show on top of cousour
weather_df %>%
  group_by(name, month)
 
# each group separately, taking the n
# new df with summary
# each month has different observations
weather_df %>%
  group_by(month) %>%
  summarize(n = n())
...
base R table function is as useful
...
weather_df %>%
  filter(name != "Waikiki_HA") %>%
  # 2 or more variables that you are interested in
  ...
  ## tabyl: a function from janitor to create a 2x2 tables.
...
```
 ...
 # grouped mutates
 ### dont forget that you can ungroup
 
```{r}
weather_df %>%
  group_by(name) %>%
  ### any thing in mutate is group specific fashion
  ### centered_tmax: is group specific tmax
  ### every group is not centered around the 0 line
...
#Window functions in grouped mutations ...
#Gives the rank
